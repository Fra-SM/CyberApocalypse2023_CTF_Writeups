...
Exploring the packet capture we are given using Wireshark, we see that there is a lot of HTTP traffic going on. Using again the Export HTTP objects functionality, Wireshark recognizes a lot of different files being transmitted. At some point, we come across the following `map_update.php` script:
